#!/bin/sh
# Taken from https://github.com/JopStro/bspswallow
...
swallow() {
	...
	if [ "$addedtodesktop" = "$lasttermdesktop" ]; then
		grep "^$(get_class "$swallowerid")$" $XDG_CONFIG_HOME/bspwm/swallow || return
		grep "^$(get_class "$swallowingid")$" $XDG_CONFIG_HOME/bspwm/terminals || return
